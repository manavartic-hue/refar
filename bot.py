...
# Railway exposes this process on PORT. Set PUBLIC_BASE_URL after generating
# the Railway public domain, for example https://my-bot.up.railway.app.
PORT = int(os.environ.get("PORT", "8080"))
PUBLIC_BASE_URL = os.environ.get("PUBLIC_BASE_URL", "").rstrip("/")
DEVICE_VERIFY_WEBAPP_URL = os.environ.get("DEVICE_VERIFY_WEBAPP_URL", "").strip() or (
    f"{PUBLIC_BASE_URL}/verify" if PUBLIC_BASE_URL else ""
)
DEVICE_BINDING_SECRET = os.environ.get("DEVICE_BINDING_SECRET", "")
VERIFY_PAGE_PATH = Path(__file__).parent / "web" / "verify.html"
...
UI_MESSAGES = {
    ...
    "device": ("🛡 <b>Device Verification</b>\n\nPlease open the secure verification page below and complete the check.\n\n<i>This checks the current Telegram session; it does not request your OTP, UPI PIN or bank password.</i>"),
...
async def save_setting_message(key: str, value: str) -> None:
    errors = validate_template_html(value)
    if errors:
        raise ValueError("; ".join(errors[:3]))
    unknown = template_variables(value) - KNOWN_TEMPLATE_VARIABLES
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            ...
            CREATE TABLE IF NOT EXISTS users (
                ...
                device_challenge   TEXT,
                device_challenge_created_at TEXT,
        ...
        # Privacy-conscious anti-fraud data: a random, per-browser binding ID
        # is hashed with a server secret. Raw browser/device fingerprints are
        # never stored. A bound ID may receive only one referral credit.
        await db.execute("""CREATE TABLE IF NOT EXISTS device_bindings (
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS user_device_bindings (
            user_id            INTEGER PRIMARY KEY,
            binding_hash       TEXT NOT NULL,
            verified_at        TEXT NOT NULL,
            UNIQUE(user_id, binding_hash)
        )""")
        await db.execute("""CREATE TABLE IF NOT EXISTS referral_decisions (
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS fraud_events (
            id                 INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id            INTEGER NOT NULL,
        ...
        for sql in (
            ...
            "ALTER TABLE users ADD COLUMN device_challenge TEXT",
            "ALTER TABLE users ADD COLUMN device_challenge_created_at TEXT",
        ):
            try:
        ...
        defaults = {
            ...
            "device_verify_webapp_url": DEVICE_VERIFY_WEBAPP_URL,
            "duplicate_device_referral_block_enabled": "1",
            "device_binding_ttl_seconds": "600",
            "task_banner_file_id": "",
        ...
        for sql in (
            ...
            "CREATE INDEX IF NOT EXISTS idx_device_bindings_first_user ON device_bindings(first_user_id)",
            "CREATE INDEX IF NOT EXISTS idx_user_device_binding_hash ON user_device_bindings(binding_hash)",
            "CREATE INDEX IF NOT EXISTS idx_referral_decisions_decision ON referral_decisions(decision, decided_at)",
...
async def _set_flag(user_id: int, column: str, value: int) -> None:
    assert column in {
        "joined_gate", "reward_sent", "phone_verified", "device_verified",
...
async def mark_device_verified(user_id: int) -> None:
    await _set_flag(user_id, "device_verified", 1)
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET device_challenge=NULL, device_challenge_created_at=NULL WHERE user_id=?",
            (user_id,),
        )
        await db.commit()


async def set_device_challenge(user_id: int, challenge: str) -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET device_challenge=?, device_challenge_created_at=? WHERE user_id=?",
            (challenge, datetime.now(timezone.utc).isoformat(), user_id),
        )
        await db.commit()


def _hash_antifraud_value(value: str) -> str:
    """Hash a short-lived client signal before writing it to the database.

    DEVICE_BINDING_SECRET must remain server-side. Raw browser identifiers, IPs,
    and user agents are deliberately not persisted.
    """
    if not DEVICE_BINDING_SECRET:
        raise RuntimeError("DEVICE_BINDING_SECRET is not configured")
    return hmac.new(
        DEVICE_BINDING_SECRET.encode("utf-8"),
        value.encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()


def validate_telegram_webapp_init_data(init_data: str, max_age_seconds: int = 900) -> dict:
    """Validate the signed Telegram Mini App initData on the server.

    Never trust initDataUnsafe or a Telegram user ID supplied by the browser.
    """
    ...
    if not hmac.compare_digest(calculated_hash, received_hash):
        raise ValueError("Telegram authorization signature is invalid")
    try:
...
async def bind_verified_device(
    user_id: int,
    binding_id: str,
    client_ip: str,
    user_agent: str,
) -> None:
    """Bind one random Mini App storage ID to a Telegram account.

    The browser-generated ID is intentionally a *risk signal*, not a claimed
    physical-device identity. It survives normal reuse of the same Telegram
    WebView, but users can reset storage or change browsers, so it is not a
    guarantee against determined abuse.
    """
    binding_hash = _hash_antifraud_value(f"binding:{binding_id}")
    ip_hash = _hash_antifraud_value(f"ip:{client_ip}") if client_ip else None
    ua_hash = _hash_antifraud_value(f"ua:{user_agent[:512]}") if user_agent else None
    now = datetime.now(timezone.utc).isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("BEGIN IMMEDIATE")
        existing = await (await db.execute(
            "SELECT first_user_id FROM device_bindings WHERE binding_hash=?", (binding_hash,)
        )).fetchone()
        if existing:
            await db.execute(
                "UPDATE device_bindings SET last_seen_at=?, last_ip_hash=?, user_agent_hash=? WHERE binding_hash=?",
                (now, ip_hash, ua_hash, binding_hash),
            )
        else:
            await db.execute(
                "INSERT INTO device_bindings(binding_hash,first_user_id,first_seen_at,last_seen_at,last_ip_hash,user_agent_hash) VALUES(?,?,?,?,?,?)",
                (binding_hash, user_id, now, now, ip_hash, ua_hash),
            )
        await db.execute(
            "INSERT INTO user_device_bindings(user_id,binding_hash,verified_at) VALUES(?,?,?) "
            "ON CONFLICT(user_id) DO UPDATE SET binding_hash=excluded.binding_hash, verified_at=excluded.verified_at",
            (user_id, binding_hash, now),
        )
        await db.execute("UPDATE users SET device_verified=1 WHERE user_id=?", (user_id,))
        await db.execute(
            "INSERT INTO fraud_events(user_id,event_type,details,created_at) VALUES(?,?,?,?)",
            (user_id, "DEVICE_SESSION_VERIFIED", "telegram_init_data_validated", now),
        )
        await db.commit()


async def get_referral_decision(user_id: int) -> tuple[str, str] | None:
    async with aiosqlite.connect(DB_PATH) as db:
        row = await (await db.execute(
            "SELECT decision, COALESCE(reason_code,'') FROM referral_decisions WHERE referred_user_id=?",
            (user_id,),
        )).fetchone()
...
async def credit_referral_and_mark(referred_user_id: int, referrer_id: int) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("BEGIN IMMEDIATE")
        cur=await db.execute("SELECT referral_credited,referred_by,banned,restricted,device_verified FROM users WHERE user_id=?",(referred_user_id,)); invited=await cur.fetchone()
        ...
        if await get_setting("device_verify_enabled", "0") == "1":
            binding_row = await (await db.execute(
                "SELECT b.binding_hash, b.first_user_id FROM user_device_bindings u "
                "JOIN device_bindings b ON b.binding_hash=u.binding_hash WHERE u.user_id=?",
                (referred_user_id,),
            )).fetchone()
            if not invited[4] or not binding_row:
                await db.execute(
                    "INSERT OR REPLACE INTO referral_decisions(referred_user_id,decision,reason_code,binding_hash,decided_at) VALUES(?,?,?,?,?)",
                    (referred_user_id, "BLOCKED", "DEVICE_NOT_VERIFIED", None, now),
                )
                await db.commit(); return False
            if await get_setting("duplicate_device_referral_block_enabled", "1") == "1" and int(binding_row[1]) != referred_user_id:
                await db.execute(
                    "INSERT OR REPLACE INTO referral_decisions(referred_user_id,decision,reason_code,binding_hash,decided_at) VALUES(?,?,?,?,?)",
                    (referred_user_id, "BLOCKED", "SAME_DEVICE_MULTIPLE_ACCOUNTS", binding_row[0], now),
                )
                await db.execute(
                    "INSERT INTO fraud_events(user_id,event_type,details,created_at) VALUES(?,?,?,?)",
                    (referred_user_id, "REFERRAL_BLOCKED", "same_device_multiple_accounts", now),
                )
                await db.commit(); return False
        try:
        ...
        await db.execute(
            "INSERT OR REPLACE INTO referral_decisions(referred_user_id,decision,reason_code,binding_hash,decided_at) VALUES(?,?,?,?,?)",
            (referred_user_id, "APPROVED", "", None, now),
        )
        await db.commit()
        return True
...
async def get_device_verify_webapp_url() -> str:
    configured = (await get_setting("device_verify_webapp_url", "")).strip()
    return DEVICE_VERIFY_WEBAPP_URL or configured


async def device_verification_enabled() -> bool:
    # A clone subprocess cannot safely share the master HTTP endpoint because
    # Telegram initData is signed with each clone's distinct bot token.
    if CLONE_MODE:
        return False
    return await get_setting("device_verify_enabled", "0") == "1" and bool(await get_device_verify_webapp_url())
...
async def next_step(user: aiosqlite.Row) -> str:
    ...
    if not user["joined_gate"]:
        if not await get_channels():
            if await device_verification_enabled() and not user["device_verified"]:
                return STEP_DEVICE
...
async def maybe_credit_referral(user_id: int, bot: Bot) -> None:
    ...
    if not credited:
        decision = await get_referral_decision(user_id)
        if decision and decision[0] == "BLOCKED" and decision[1] == "SAME_DEVICE_MULTIPLE_ACCOUNTS":
            try:
                await bot.send_message(
                    user_id,
                    "⚠️ <b>Referral not counted</b>\n\n"
                    "This Telegram account is using a device/session already linked to another account. "
                    "Your bot access remains available, but this referral cannot earn a reward.",
                )
            except Exception:
                pass
        return
...
async def protected_feature_for_event(event, data) -> str:
    ...
    prefixes = (
        ...
        ("vs_captcha", "captcha"), ("vs_phone", "phone_verification"), ("vs_device", "verification"),
...
def state_feature(state_name: str | None) -> str:
    ...
    return {
        ...
        "waiting_maintenance_message":"system",
...
def device_keyboard(url: str) -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text=UI_BUTTONS["verify_device"], web_app=WebAppInfo(url=url))]],
...
def verification_settings_keyboard(captcha_on: bool, phone_on: bool, device_on: bool, duplicate_block_on: bool) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(
                text=f"🛡 Device: {'✅ ON' if device_on else '❌ OFF'}",
                callback_data="vs_device",
            )],
            [InlineKeyboardButton(
                text=f"🚫 Same-device referral: {'✅ BLOCK' if duplicate_block_on else '⚠️ ALLOW'}",
                callback_data="vs_duplicate_device",
            )],
            [InlineKeyboardButton(text="📝 Edit Device Message", callback_data="vs_edit_device")],
...
def user_card(user: aiosqlite.Row, required: int) -> tuple[str, InlineKeyboardMarkup]:
    ...
    text = (
        ...
        f"🛡 Device/session: {'verified' if user['device_verified'] else 'not verified'}\n"
...
async def render_device(bot: Bot, chat_id: int, user_id: int, edit_message: Optional[Message] = None) -> None:
    url = await get_device_verify_webapp_url()
    if not url:
        await bot.send_message(chat_id, "⚠️ Device verification is not configured yet. Please contact the admin.")
        return
    text = await ui_message("device")
    await delete_previous_flow(bot, user_id, chat_id)
    msg = await bot.send_message(chat_id, text, reply_markup=device_keyboard(url))
...
async def render_flow(bot: Bot, chat_id: int, user_id: int, edit_message: Optional[Message] = None) -> None:
    ...
    elif step==STEP_DEVICE: await render_device(bot,chat_id,user_id,edit_message=edit_message)
...
async def on_device_web_app_data(message: Message, bot: Bot) -> None:
    user_id = message.from_user.id
    user = await get_user(user_id)
    if user is None or user["banned"] or user["restricted"]:
        return
    if not await device_verification_enabled():
        return
    try:
        payload = json.loads((message.web_app_data.data or "").strip())
    except json.JSONDecodeError:
        await message.answer("❌ Verification response is invalid. Please try again.")
        await render_flow(bot, message.chat.id, user_id)
        return
    if not isinstance(payload, dict) or payload.get("status") != "verified":
        await message.answer("❌ Server verification was not completed. Please try again.")
        return
    user = await get_user(user_id)
    if not user or not user["device_verified"]:
        await message.answer("❌ Verification must be completed in the secure page first.")
        await render_flow(bot, message.chat.id, user_id)
        return
    await message.answer("✅ Session verification complete.")
...
async def _render_verification_settings(message: Message) -> None:
    ...
    duplicate_block_on = await get_setting("duplicate_device_referral_block_enabled", "1") == "1"
    url_ready = bool(await get_device_verify_webapp_url())
    await message.edit_text(
        ...
        f"🛡 <b>Device/session</b> — {'configured' if url_ready else 'Railway URL missing'}.\n"
        f"🚫 <b>Same-device referrals</b> — {'blocked' if duplicate_block_on else 'allowed'}.",
        reply_markup=verification_settings_keyboard(captcha_on, phone_on, device_on, duplicate_block_on),
    )
...
async def cb_toggle_device(callback: CallbackQuery) -> None:
    if CLONE_MODE:
        await callback.answer("Device verification is supported on the master bot only.", show_alert=True)
        return
    if not await get_device_verify_webapp_url():
        await callback.answer("Set DEVICE_VERIFY_WEBAPP_URL in Railway first.", show_alert=True)
        return
    current = await get_setting("device_verify_enabled", "0")
    await set_setting("device_verify_enabled", "0" if current == "1" else "1")
    await _render_verification_settings(callback.message)
    await callback.answer("Device/session verification updated.")


@admin_router.callback_query(F.data == "vs_duplicate_device")
async def cb_toggle_duplicate_device_block(callback: CallbackQuery) -> None:
    current = await get_setting("duplicate_device_referral_block_enabled", "1")
    await set_setting("duplicate_device_referral_block_enabled", "0" if current == "1" else "1")
    await _render_verification_settings(callback.message)
    await callback.answer("Same-device referral rule updated.")


@admin_router.callback_query(F.data == "vs_edit_device")
async def cb_edit_device_message(callback: CallbackQuery, state: FSMContext) -> None:
    current = await ui_message("device")
    await state.update_data(ui_key="device")
    await state.set_state(AdminStates.waiting_ui_message)
    await callback.message.edit_text(
        "📝 <b>Edit: Device Verification Message</b>\n\n"
        f"<b>Current preview:</b>\n\n{current}\n\n"
        "Send the new message now. HTML formatting is supported.\nUse /cancel to leave without changing it.",
        reply_markup=await cancel_keyboard("adm_verify"),
    )
    await callback.answer()
...
async def cb_admin_system(callback: CallbackQuery) -> None:
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="📝 Edit Maintenance Message", callback_data="sys_edit_maintenance")],
...
@admin_router.callback_query(F.data == "sys_edit_maintenance")
async def cb_edit_maintenance_message(callback: CallbackQuery, state: FSMContext) -> None:
    current = await get_setting("maintenance_message", "🛠 <b>Temporarily Under Maintenance</b>")
    await state.set_state(AdminStates.waiting_maintenance_message)
    await callback.message.edit_text(
        "📝 <b>Edit: Maintenance Message</b>\n\n"
        f"<b>Current preview:</b>\n\n{current}\n\n"
        "Send the new message now. HTML formatting is supported.\n"
        "Available variable: <code>{bot_name}</code>\nUse /cancel to leave without changing it.",
        reply_markup=await cancel_keyboard("adm_system"),
    )
    await callback.answer()


@admin_router.message(AdminStates.waiting_maintenance_message)
async def process_maintenance_message(message: Message, state: FSMContext) -> None:
    value = message.html_text if message.text else (message.caption or "")
    if not value.strip():
        await message.answer("❌ Send a text message (or a photo with caption) to save it.")
        return
    old_value = await get_setting("maintenance_message", "")
    try:
        await save_setting_message("maintenance_message", value)
    except ValueError as exc:
        await message.answer(f"⚠️ <b>Template rejected</b>\n\n<code>{hesc(str(exc))}</code>")
        return
    await audit(message.from_user.id, "CHANGE_MAINTENANCE_MESSAGE", before=old_value, after=value)
    await state.clear()
    await message.answer("✅ <b>Maintenance message updated.</b>", reply_markup=await admin_panel_keyboard(message.from_user.id))
...
# ---------------------------------------------------------------------------
# Railway HTTP service — Mini App page and server-validated binding endpoint.
# This runs in the same process as bot polling and must remain on HTTPS.
# ---------------------------------------------------------------------------

web_app = FastAPI(title="Telegram referral verification", docs_url=None, redoc_url=None)


@web_app.get("/health")
async def http_health() -> dict:
    return {"ok": True, "service": "telegram-referral-verification"}


@web_app.get("/verify")
async def http_verify_page() -> FileResponse:
    if not VERIFY_PAGE_PATH.is_file():
        raise HTTPException(status_code=503, detail="Verification page is not deployed")
    return FileResponse(VERIFY_PAGE_PATH, media_type="text/html; charset=utf-8")


@web_app.post("/api/verify")
async def http_verify_device(request: Request) -> JSONResponse:
    """Validate Telegram's signed initData and bind one random webview ID.

    The endpoint never trusts a Telegram user ID supplied by the browser.
    It also never stores the raw binding ID, IP, or user-agent string.
    """
    if not DEVICE_BINDING_SECRET:
        raise HTTPException(status_code=503, detail="Server verification is not configured")
    try:
    ...
    if not init_data or not re.fullmatch(r"[A-Za-z0-9_-]{32,128}", binding_id):
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail="Telegram session could not be validated") from exc

    user = await get_user(user_id)
    if user is None or user["banned"] or user["restricted"]:
        raise HTTPException(status_code=403, detail="Account is not eligible for verification")
    client_ip = (request.headers.get("x-forwarded-for", "").split(",")[0].strip() or request.client.host)
    await bind_verified_device(
        user_id=user_id,
...
async def start_http_service() -> tuple[uvicorn.Server, asyncio.Task]:
    config = uvicorn.Config(web_app, host="0.0.0.0", port=PORT, log_level="warning", lifespan="off")
    server = uvicorn.Server(config)
    task = asyncio.create_task(server.serve())
    # Give Uvicorn a short chance to bind. Railway will continue health checks.
    await asyncio.sleep(0.15)
    if task.done():
        await task
    logger.info("HTTP verification service listening on port %s", PORT)
    return server, task
...
async def main() -> None:
    ...
    http_task = None
    if not CLONE_MODE:
        http_server, http_task = await start_http_service()
