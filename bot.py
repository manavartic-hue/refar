...
DEFAULT_HOLWIN_INVITE = os.environ.get("DEFAULT_HOLWIN_INVITE", "WLRPSY")
DEFAULT_REX_INVITE    = os.environ.get("DEFAULT_REX_INVITE",    "O6NVYX")

# Holwin & Rex API endpoints
...
# ─────────────────────────── MarkdownV2 escaping ───────────────────────────
# FIX: original regex was missing the '[' character and had a malformed pattern.
# All 18 reserved MarkdownV2 characters properly escaped here.
...
class ProxyManager:
    ...
    async def get_next_proxy(self) -> Optional[str]:
        """Returns proxy URL as http://user:pass@host:port, rotating round-robin."""
        if not self.proxies:
            return None
    ...
    def reload(self, new_lines: List[str]) -> int:
        new_proxies = []
        ...
        if new_proxies:
            ...
            self.current_index = 0
            logger.info(f"Proxy list reloaded: {len(self.proxies)} proxies.")
        else:
            logger.warning("No valid proxies in reload — keeping old list.")
        return len(self.proxies)


# Global proxy manager — populated in main()
...
# ─────────────────────────── i18n ───────────────────────────
# NOTE: help text is stored as pre-escaped MarkdownV2 literals.
# Dynamic data must still pass through esc() before insertion.
STRINGS: Dict[str, Dict[str, str]] = {
    ...
    "banned": {
        ...
        "hi": "🚫 आपको इस बॉट का उपयोग करने से प्रतिबंधित किया गया है।",
    },
    "enter_mobile": {
    ...
    "invalid_mobile": {
        "en": "❌ Invalid number. Enter 10–15 digits only:",
        "hi": "❌ अमान्य नंबर। केवल 10–15 अंक दर्ज करें:",
    },
}
...
def save_registration(mobile: str, platform: str, invite: str, telegram_id: int) -> None:
    db: Session = db_session()
    try:
        db.add(Registration(
            mobile=mobile, platform=platform,
            invite_used=invite, telegram_id=telegram_id,
        ))
        db.commit()
...
def get_invite_code(platform: str) -> str:
    ...
    try:
        ...
        if config:
            return config.value
        return DEFAULT_HOLWIN_INVITE if platform == "holwin" else DEFAULT_REX_INVITE
    finally:
        db.close()
...
class BaseAPIClient:
    def __init__(self, base_url: str, headers: Dict[str, str], proxy_manager: ProxyManager):
    ...
    async def _post_with_retry(
        ...
        for attempt in range(1, retries + 1):
            proxy_url = await self.proxy_manager.get_next_proxy()
            try:
                timeout = aiohttp.ClientTimeout(total=20)
                # ssl=False only if proxy is HTTP and target is HTTPS —
                # keeps compatibility while avoiding ssl verify errors on some proxies.
                connector = aiohttp.TCPConnector(ssl=False) if proxy_url else aiohttp.TCPConnector()
                ...
                last_error = e
                logger.warning(f"Request attempt {attempt} failed (proxy={proxy_url}): {e}")
            except asyncio.TimeoutError:
                last_error = "Timeout"
                logger.warning(f"Timeout attempt {attempt} (proxy={proxy_url})")
            except Exception as e:
                last_error = e
...
async def _send_or_edit(
    ...
    """Send a new message or edit the current one depending on context."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if info["is_banned"]:
        await _send_or_edit(update, L("banned", lang))
        return

    holwin_invite = get_invite_code("holwin")
    rex_invite    = get_invite_code("rex")

    # FIX: every dynamic value (invite codes) passed through esc().
    # Static box-drawing chars are safe; they contain no MarkdownV2 reserved chars.
    msg = (
        "╔═══════════════════════════════╗\n"
        f"║   {L('main_title', lang)}   ║\n"
        "╚═══════════════════════════════╝\n\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _send_or_edit(update, text, reply_markup=back_keyboard(), parse_mode=ParseMode.MARKDOWN_V2)


async def language_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    info = await touch_user_and_check_ban(update)
    # lang_prompt has no MarkdownV2 formatting needed — plain text is safer
    text = L("lang_prompt", info["language"])
    await _send_or_edit(update, text, reply_markup=language_keyboard())
...
async def set_language(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # lang_set is plain text — no MarkdownV2 needed
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # FIX: build plain-text stats — no MarkdownV2 needed here, avoids escaping issues
    # with dates (which contain '-' and ':').
    lines = [
        "📊 Global Stats\n",
        f"👥 Total: {total}",
        f"🏠 Holwin: {holwin}",
    ...
    if recent:
        for r in recent:
            lines.append(
                f"• {r.mobile} - {r.platform.upper()} - {r.registered_at.strftime('%Y-%m-%d %H:%M')}"
            )
    else:
        lines.append("No registrations yet.")
...
async def referral_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # FIX: esc() applied to the link (contains '.' and '-')
    caption = (
        "🔗 *Your Referral Link*\n\n"
        f"`{esc(link)}`\n\n"
...
async def support_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if args:
        ...
        # Plain text — FAQ answers may contain chars that would need escaping
        text = f"🆘 {answer}" if answer else "🤔 No FAQ match found."
    else:
        text = (
            "🆘 Quick Support\n\n"
            "Type a keyword (e.g. otp, password, error) after /support, "
            "or just ask your question as a normal message."
        )

    target = update.callback_query.message if update.callback_query else update.message
    if update.callback_query:
        await update.callback_query.answer()
    # Plain text to avoid escaping issues in FAQ answer strings
    await target.reply_text(text, reply_markup=back_keyboard())


async def freeform_text_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text   = update.message.text or ""
    answer = match_faq(text)
    if answer:
        # Plain text — safe for all FAQ content
        await update.message.reply_text(f"🆘 {answer}", reply_markup=back_keyboard())

# ─────────────────────────── Admin: invite management ───────────────────────────

async def set_invite_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if set_invite_code(platform, code):
        # FIX: plain text — no MarkdownV2 parse_mode; platform/code are dynamic and could contain special chars
        await update.message.reply_text(f"✅ Invite code for {platform} set to: {code}")
    else:
        await update.message.reply_text("❌ Failed to set invite code.")


async def show_invites_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    holwin = get_invite_code("holwin")
    rex    = get_invite_code("rex")
    # Plain text — invite codes are dynamic values
    await update.message.reply_text(f"🏠 Holwin: {holwin}\n📈 Rex: {rex}")


async def reset_invites_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
...
async def reload_proxies_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    await update.message.reply_text(
        "To reload proxies, place a 'proxies.txt' file with one proxy per line "
        "and use /load_proxies_file, or restart the bot with an updated PROXY_LINES."
    )


async def load_proxies_file_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    if not os.path.exists("proxies.txt"):
        await update.message.reply_text("File 'proxies.txt' not found in working directory.")
        return
    ...
    if not lines:
        ...
        return
    count = PROXY_MANAGER.reload(lines)
    await update.message.reply_text(f"✅ Reloaded {count} proxies from proxies.txt.")
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    proxy_idx     = PROXY_MANAGER.current_index
    # Plain text — all values are integers/numbers, safe without MarkdownV2
    await update.message.reply_text(
        f"📊 Bot Status\n"
        f"👥 Users: {total_users}\n"
        f"📝 Registrations: {total_reg} (Holwin: {holwin}, Rex: {rex})\n"
        f"🌐 Proxies loaded: {proxy_count}\n"
        f"🔄 Current proxy index: {proxy_idx}"
    )

# ─────────────────────────── Admin: broadcast / users / ban / export ───────────────────────────
...
async def send_summary(context: ContextTypes.DEFAULT_TYPE, label: str) -> None:
    total, holwin, rex, _ = get_stats()
    # FIX: plain text — label/numbers are safe; avoids MarkdownV2 errors in scheduled jobs
    text = (
        f"📈 {label} Summary\n\n"
...
async def platform_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    platform = q.data.split("_")[1]   # platform_holwin or platform_rex
    invite   = get_invite_code(platform)
    context.user_data["platform"] = platform
    context.user_data["invite"]   = invite

    # FIX: esc() applied to both dynamic values
    await q.edit_message_text(
        f"✅ Selected: *{esc(platform.upper())}*\n"
        f"Invite: `{esc(invite)}`\n\n"
        f"{esc(L('enter_mobile', info['language']))}",
        parse_mode=ParseMode.MARKDOWN_V2,
        reply_markup=back_keyboard(),
    )
    return MOBILE
...
async def mobile_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    info   = await touch_user_and_check_ban(update)
    ...
    ok = (platform == "holwin" and resp.get("code") == 0) or \
         (platform == "rex"    and resp.get("code") == 200)

    if not ok:
        # Plain text — API msg may contain MarkdownV2 special chars
        await update.message.reply_text(
...
async def otp_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Plain text — no MarkdownV2 needed for this prompt
    await update.message.reply_text("🔑 Set a password (6+ chars), or type skip:")
    return PASSWORD
...
async def password_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # FIX: esc() on all dynamic values; '*' * len(pwd) is safe (only asterisks)
    summary = (
        "📋 *Summary*\n\n"
        f"📱 Mobile: `{esc(mobile)}`\n"
        f"🔑 Password: `{'*' * len(pwd)}`\n"
...
async def change_mobile(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    q = update.callback_query
    await q.answer()
    await q.edit_message_text(
        "✏️ Enter your new mobile number (10–15 digits):",
        reply_markup=back_keyboard(),
    )
    return MOBILE


async def confirm_reg(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    q = update.callback_query
    await q.answer()
    ...
    if success:
        try:
            ...
            await q.edit_message_text("❌ Registration succeeded on platform but local save failed.")
            return ConversationHandler.END

        # FIX: all dynamic values escaped; literal '!' escaped as '\!'
        await q.edit_message_text(
    ...
    # Failure — plain text to safely display API error message
    await q.edit_message_text(
        f"❌ Registration failed: {resp.get('msg', 'Unknown error')}",
        reply_markup=back_keyboard(),
    )
    return ConversationHandler.END
...
conv_handler = ConversationHandler(
    ...
    states={
        ...
        CONFIRM: [
            ...
            CallbackQueryHandler(change_mobile, pattern=r"^change_mobile$"),
            CallbackQueryHandler(cancel,        pattern=r"^cancel_reg$"),
            CallbackQueryHandler(main_menu,     pattern=r"^main_menu$"),
        ],
    },
    fallbacks=[
...
def main() -> None:
    if not BOT_TOKEN or BOT_TOKEN.count(":") != 1:
        raise SystemExit(
            ...
            "Set it via the BOT_TOKEN environment variable."
        )
    if not ADMIN_IDS:
        logger.warning("No ADMIN_IDS configured — admin commands will be unusable.")

    # ── Proxy initialisation ──────────────────────────────────────────────────
    # Priority 1: proxies.txt file (easiest to update without redeployment)
    # Priority 2: PROXY_LINES constant below
    proxy_lines: List[str] = []

    if os.path.exists("proxies.txt"):
        with open("proxies.txt", "r", encoding="utf-8") as fh:
            proxy_lines = [line.strip() for line in fh if line.strip()]
        logger.info(f"Loaded {len(proxy_lines)} proxy lines from proxies.txt.")
    else:
        # ── Paste your full proxy list here ───────────────────────────────────
        # Format: host:port:username:password  (one per line as a string element)
        proxy_lines = [
            ...
            # Add remaining proxies here …
        ]

    global PROXY_MANAGER
    PROXY_MANAGER = ProxyManager(proxy_lines)
    logger.info(f"Proxy manager ready: {len(PROXY_MANAGER.proxies)} proxies loaded.")
    ...
    # Registration conversation (must be before generic callback handlers)
    app.add_handler(conv_handler)

    # Inline button callbacks (outside conversation)
    app.add_handler(CallbackQueryHandler(stats_cmd,    pattern=r"^stats_btn$"))
    app.add_handler(CallbackQueryHandler(my_cmd,       pattern=r"^my_btn$"))
    app.add_handler(CallbackQueryHandler(help_cmd,     pattern=r"^help_btn$"))
    app.add_handler(CallbackQueryHandler(referral_cmd, pattern=r"^referral_btn$"))
    app.add_handler(CallbackQueryHandler(language_cmd, pattern=r"^lang_btn$"))
    app.add_handler(CallbackQueryHandler(set_language, pattern=r"^setlang_(en|hi)$"))
    app.add_handler(CallbackQueryHandler(support_cmd,  pattern=r"^support_btn$"))
    app.add_handler(CallbackQueryHandler(main_menu,    pattern=r"^main_menu$"))

    # Freeform text fallback (group=1 runs after conversation group=0)
    app.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, freeform_text_fallback),
        group=1,
    )
    ...
    if app.job_queue is not None:
        ...
        logger.info("Scheduled daily and weekly summary jobs registered.")
    else:
        logger.warning(
            "JobQueue unavailable — install python-telegram-bot[job-queue] for scheduled summaries."
        )

    app.add_error_handler(error_handler)

    logger.info("Premium Referral Bot with proxy rotation started.")
